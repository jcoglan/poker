export function oddsExact(total, hits, turns, wins) {
...
export function oddsAtLeast(total, hits, turns, min) {
  let odds = new Rational(0);
  for (let i = min; i <= turns; i++) odds = odds.add(oddsExact(total, hits, turns, i));
  return odds.normalize();
}
